...
export default function Experience() {
  return (
    <Section id="experience" num="02" title="Where I've worked">
      <ol className="relative border-l border-white border-opacity-10 ml-3 md:ml-6 space-y-12 font-sans">
        {experience.map((job, idx) => (
          <li 
            key={job.company} 
            className="relative pl-8 md:pl-12 reveal-item reveal-fade-left"
          ...
          >
            ...
            <SpotlightCard className="glass-panel glass-panel-hover rounded-2xl p-6 md:p-8 border border-white border-opacity-5">
              ...
              <ul className="mt-5 flex flex-wrap gap-2">
                {job.tech.map((t) => (
                  <li key={t} className="text-xs font-semibold text-accent bg-accent bg-opacity-10 border border-accent border-opacity-15 rounded-full px-3 py-1 hover:bg-accent hover:bg-opacity-20 transition-colors duration-200 cursor-default">
                    {t}
                  </li>
                ))}
              </ul>
